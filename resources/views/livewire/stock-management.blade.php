<div>
    ...
    <div>
        @php
            $jumlahBarangAkanHabis = 0;
            foreach ($items as $item){
                if ($item->stok <= 10) {
                    $jumlahBarangAkanHabis++;
                } else {
                    $showAlert = false;
                }  
            }
        @endphp
        @if ($jumlahBarangAkanHabis > 0)
            
        <p class="text-red-800 text-xl mt-5 font-bold">Terdapat {{ $jumlahBarangAkanHabis }}  Barang Akan Habis, Segara Update!
        </p>
        @endif
    </div>
    <!-- Products List -->
    <div class="mt-6 bg-green-50 rounded-lg overflow-hidden">
        ...
        <div class="divide-y divide-green-100">
            @foreach ($items as $item)
    <div class="grid grid-cols-12 gap-4 py-4 px-6 items-center {{ $item->stok <= 10 ? 'bg-red-100 hover:bg-red-200' : 'hover:bg-green-50' }}">
